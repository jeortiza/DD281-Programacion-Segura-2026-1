...
- X) Cada petición HTTP es independiente y el servidor no recuerda peticiones anteriores
...
- X) Cookie con atributos `HttpOnly` y `Secure`
...
- X) Impide que JavaScript del navegador pueda leer el valor de la cookie
...
- X) La cookie solo se transmite sobre conexiones HTTPS, nunca HTTP
...
- X) Un atacante con una copia previa del session ID puede seguir usándolo para acceder al sistema
...
- X) CSRF (Cross-Site Request Forgery) desde dominios externos
...
- X) Fuerza al usuario a utilizar un session ID ya conocido por el atacante antes de autenticarse
...
- X) Cada usuario debe tener únicamente los permisos estrictamente necesarios para su función
...
- X) Parameter tampering — el usuario puede editar el campo con DevTools y darse el rol "admin"
...
- X) El navegador rechaza y descarta la cookie automáticamente
...
1. HTTP es un protocolo _STATELESS_ porque no recuerda peticiones anteriores entre cliente y servidor.

2. El atributo SECURE garantiza que la cookie de sesión no sea transmitida sobre conexiones HTTP no cifradas.

3. El modelo de control de acceso _RBAC_ asigna permisos a través de roles, no directamente a usuarios individuales.
...
| 5. Session Fixation | __D__ | e) Robo de un session ID válido para suplantar al usuario |
| 6. Stateless | __B__ | f) La cookie solo viaja sobre conexiones HTTPS |
| 7. Mínimo Privilegio | __H__ | g) Conjunto de permisos asignados a un tipo de usuario |
...
Guardar el session ID en localStorage es un grave riesgo de seguridad porque cualquier script JavaScript ejecutado en el navegador (incluyendo código malicioso inyectado mediante XSS) puede leer y robar ese valor. Esto permite a un atacante secuestrar la sesión del usuario sin necesidad de interceptar la comunicación. La alternativa correcta es almacenar el session ID en una cookie con el atributo HttpOnly, que impide que JavaScript acceda a su contenido, garantizando que solo el navegador la envíe automáticamente en cada petición al servidor. Además, debe incluir el atributo Secure para que solo se transmita por HTTPS y SameSite para prevenir ataques CSRF. Este enfoque sigue el principio de defensa en profundidad, protegiendo la sesión tanto en el cliente como en la red.
...
El Session Hijacking y el Session Fixation son ataques que buscan la suplantación de identidad, pero difieren en su mecánica. El Hijacking consiste en robar un session ID ya establecido y válido mediante técnicas como interceptación de tráfico, XSS o malware; el atacante obtiene la sesión activa del usuario legítimo. En cambio, en el Session Fixation, el atacante impone al usuario un session ID específico antes de que se autentique, luego espera que el usuario inicie sesión con ese ID y así poder usarlo después. Para prevenir Hijacking, se deben usar cookies con atributos HttpOnly y Secure, además de cifrar las comunicaciones con HTTPS. Contra Fixation, la medida efectiva es regenerar el session ID inmediatamente después de una autenticación exitosa mediante session.clear() o session.regenerate(), invalidando el ID previo y creando uno nuevo.
...
RetailFácil tiene múltiples vulnerabilidades críticas. Primero, almacena el rol del usuario en una cookie sin cifrar y sin integridad, lo que permite que un atacante modifique el valor role=comprador a role=admin mediante técnicas de manipulación de cookies. Segundo, envía el precio como campo oculto en el formulario, lo que permite que el usuario modifique el valor antes del envío usando DevTools, pudiendo pagar menos o incluso valores negativos. Tercero, la sesión sin tiempo de expiración permite que una cookie robada sea utilizada indefinidamente. Además, si el sistema no usa HTTPS, las cookies viajan en texto plano y pueden ser interceptadas en redes públicas mediante ataques de hombre en el medio.
...
La reimplementación segura debe aplicar el Principio de Confianza Cero: nunca confiar en datos provenientes del cliente. El rol del usuario debe almacenarse exclusivamente en el servidor, asociado al session ID en una base de datos o caché segura. Los precios deben validarse en el servidor consultando la base de datos por el precio real del producto, nunca usando el valor enviado desde el cliente. Las cookies deben configurarse con HttpOnly, Secure, SameSite=Strict y un tiempo de expiración corto (ej. 30 minutos). Además, se debe implementar un mecanismo de "logout" que invalide la sesión en el servidor y regenerar el session ID en cada cambio de privilegio o autenticación.
...
Se manifiesta porque un médico puede acceder a la historia clínica de cualquier paciente cambiando el parámetro paciente_id en la URL. No hay validación de permisos ni control de acceso.
...
El sistema funciona sobre HTTP sin redirigir a HTTPS, exponiendo credenciales y cookies en texto claro durante la transmisión.
...
A03:2021 – Injection (potencial)
Aunque no se menciona explícitamente, el acceso directo por parámetro sugiere que podría haber riesgo de manipulación de parámetros o SQL Injection si no se usan consultas parametrizadas.
...
Aunque HttpOnly y Secure protegen contra XSS y transmisión insegura, no sirven de nada si el identificador de sesión es predecible. El ataque habilitado es predicción de sesión, y la defensa correcta es usar IDs aleatorios con alta entropía criptográfica.
