...
# ─── USUARIOS DE PRUEBA (en producción: usar BD con bcrypt) Strict
USUARIOS = {
...
def login():
    if request.method == 'POST':
        ...
        if user and user['password'] == password:
            ...
            # ___Limpiar la sesiòn, antes de  asignar usuarios____________
...
def logout():
    ...
    #  limpiar sesiòn y eliminar cookie:
    pass
...
*Tu respuesta:* _________app.py____________________________________

**P1.2:** ¿Qué pasa si cambias el atributo `SESSION_COOKIE_HTTPONLY` a `False`? Observa qué cambia en DevTools.

*Tu respuesta:* la cookie se vuelve accesible desde JavaScript. En DevTools, se puede observar que ahora la cookie aparece en document.cookie

**P1.3:** Abre la consola de DevTools y ejecuta `document.cookie`. ¿Qué muestra? ¿Por qué?

*Tu respuesta:* Cuando SESSION_COOKIE_HTTPONLY=True, document.cookie NO muestra la cookie de sesión. Esto se debe a que el atributo HttpOnly impide que JavaScript acceda a la cookie, protegiéndola contra ataques XSS. La cookie solo es enviada automáticamente por el navegador en cada petición HTTP.
...
*Tu respuesta:* Si el rol se leyera del cliente mediante request.args.get('role'), un atacante podría explotar esto fácilmente mediante un ataque de Parameter Tampering. Simplemente agregaría ?role=admin a la URL o modificaría el campo oculto del formulario usando DevTools. Por ejemplo, un usuario normal podría acceder a http://localhost:5000/admin/panel?role=admin y ganar acceso a funciones de administrador. La seguridad debe basarse en datos del servidor (la sesión), no en datos enviados por el cliente, ya que estos pueden ser manipulados por el usuario.
...
*Tu respuesta:* El error 403 muestra un mensaje genérico de "Acceso denegado" y los roles requeridos. No revela información sensible como configuración del servidor, estructura de base de datos, rutas de archivos o detalles internos. Esto es seguro porque solo informa al usuario que no tiene permisos suficientes, sin exponer información técnica que podría ayudar a un atacante a planear un ataque más sofisticado. El mensaje es informativo pero limitado, cumpliendo con el principio de "fail securely"
...
*Tu observación:* Sí, el session_id CAMBIA completamente después del login. Esto se debe a que en el login se ejecuta session.clear(), que:

Regenera el session ID

Elimina todos los datos de sesión previos

Crea un nuevo ID con alta entropía
...
✅ Registro de sesiones al hacer login (en /login)

✅ Actualización de 'last_seen' en cada petición (en el decorador require_role)

✅ Ruta /admin/sesiones-activas (solo admin) para ver sesiones activas
---
...
El concepto más difícil de implementar fue el manejo correcto de la auditoría de sesiones activas, porque requiere coordinar múltiples partes del sistema: registro al login, actualización en cada petición, y visualización para administradores. También fue desafiante entender completamente cómo funciona la regeneración del session ID con session.clear() para prevenir Session Fixation, y cómo el decorador require_role debe leer el rol del servidor (session) y no del cliente.
